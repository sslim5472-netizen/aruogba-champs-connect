...
const refereeAuthSchema = z.object({
...
const RefereeLogin = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = refereeAuthSchema.safeParse({ email, password });
      if (!result.success) {
      ...
      if (signInError) {
        if (signInError.message.includes('Invalid login credentials')) {
          toast.error("Invalid referee credentials. Please try again.");
        } else {
        ...
        <div className="glass-card p-8 rounded-xl animate-fade-in border-2 border-primary/20">
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground">
              Restricted access - Referees only
            </p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <Label htmlFor="email">Referee Email</Label>
              <Input
                id="email"
                type="email"
                value={email}
